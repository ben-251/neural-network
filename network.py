...
class Network:
	def __init__(self,layer_sizes: Iterable[int], learning_rate:float|None=None, isTesting:bool|None=None, isZeroed: bool|None = None):
		'''
		...
		'''
		self.initialise_layers(layer_sizes,isZeroed=isZeroed)
		self.learning_rate = learning_rate
		self.isTesting = True if isTesting else False # Kept this way to handle none automatically
	
	def initialise_layers(self, layer_sizes: Iterable[int],isZeroed:bool|None):
		'''
		...
		for i, size in enumerate(layer_sizes):
			if i == 0:
				...
				self.layers.append(WeightedLayer(self.layers[-1], layer_size=size, isZeroed = isZeroed)) # watch out for index problems
	...
	def storeWeightsAndBiases(self):
		...
		with open(FILE_PATH, "w") as f:
			...
			for i, layer in enumerate(self.layers):
				...
				new_entry["weights"] = data_handler.encode_matrix(layer.weights)
				new_entry["biases"] = data_handler.encode_matrix(layer.biases)
